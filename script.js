...
const SUPABASE_URL = 'https://lsrsoznfbsxxbjdazkmv.supabase.co';
const SUPABASE_KEY = 'sb_publishable_jgl0dVGnrUuqts-fEbzphg_uck2nLAj';
...
const CONTACT_EMAIL = 'anaoliverr@icloud.com';
...
const WHATSAPP_NUMBER = '554195427409';
...
/* =====================================================================
   7. LISTA DE PRESENTES INTERATIVA
   - GIFTS é a lista fixa de presentes disponíveis.
   - As reservas ficam salvas no Supabase.
   - O formato usado pelo site continua sendo:
     {
       idDoPresente: {
         name,
         date
...
/**
 * Carrega todas as reservas do Supabase.
 */
async function loadReservations() {
  try {
    const { data, error } = await supabaseClient
      .from('reservas_presentes')
      .select('presente_id, name, date');
    ...
    data.forEach(function (reserva) {
      reservations[reserva.presente_id] = {
        name: reserva.name || '',
        date: reserva.date
      };
    });
...
/**
 * Salva uma nova reserva no Supabase.
 */
async function saveReservation(giftId, reservation) {
  try {
    ...
    // Guarda o token somente neste navegador
    localStorage.setItem(
      `reserva-token-${giftId}`,
      cancelToken
    );
...
/**
 * Cancela uma reserva somente se este navegador possuir o token.
 */
async function deleteReservation(giftId) {
  try {
    const cancelToken = localStorage.getItem(
      `reserva-token-${giftId}`
    );

    if (!cancelToken) {
      alert('Esta reserva não pode ser cancelada neste navegador.');
      return false;
    }
    ...
    if (!data) {
      alert('Esta reserva não pertence a este navegador.');
      return false;
    }

    // Remove o token deste navegador
...
/**
 * Monta o HTML de um único card de presente.
 */
function renderGiftCard(gift, reservation, isFormOpen, canCancel) {
  ...
  <button
    type="button"
    class="btn-ghost gift-unreserve"
...
/**
 * Liga toda a funcionalidade da lista de presentes.
 */
async function initGiftRegistry() {
  const grid = document.getElementById('giftGrid');

  if (!grid) return;

  // Carrega as reservas diretamente do Supabase
...
function render() {
  grid.innerHTML = GIFTS.map(function (gift) {
    return renderGiftCard(
      ...
      gift.id === openFormId,
      !!localStorage.getItem(`reserva-token-${gift.id}`)
    );
  }).join('');
}
   ...
   */
  async function confirmReservation(giftId) {

    const gift = GIFTS.find(function (g) {
      return g.id === giftId;
    });
    ...
    // Mostra um pequeno estado de carregamento
    const confirmButton = grid.querySelector(
      `button[data-action="confirm"][data-gift-id="${giftId}"]`
    );
    ...
    // Abre o WhatsApp
    const message =
      'Oi Felipe e Ana! Aqui é ' +
      (guestName || 'um convidado') +
      '. ' +
      'Quero presentear vocês com: ' +
      (gift ? gift.name : 'um presente da lista') +
      ' 🎁';
  ...
  async function unreserveGift(giftId) {
    ...
    if (!window.confirm(
      'Cancelar a reserva deste presente?'
    )) {
      return;
    }


    const deleted = await deleteReservation(giftId);
  ...
  grid.addEventListener('keydown', function (event) {
    ...
    if (
      event.target.matches('input[data-gift-id]') &&
      event.key === 'Enter'
    ) {
